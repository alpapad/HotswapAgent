...
public class AttributeInfo {
		...
		if (nameStr.charAt(0) < 'L') {
			...
			if (nameStr.equals(org.hotswap.agent.javassist.bytecode.LineNumberAttribute.tag)) {
				...
				return new org.hotswap.agent.javassist.bytecode.AnnotationsAttribute(cp, name, in);
					...
					|| nameStr
							.equals(org.hotswap.agent.javassist.bytecode.ParameterAnnotationsAttribute.invisibleTag)) {
	...
	static ArrayList<AttributeInfo> copyAll(ArrayList<AttributeInfo> list, org.hotswap.agent.javassist.bytecode.ConstPool cp) {
		if (list == null) {
			return null;
		}
