...
public class Type {
	 ...
	 */
	public int getSize() {
		return clazz == org.hotswap.agent.javassist.CtClass.doubleType
				|| clazz == org.hotswap.agent.javassist.CtClass.longType || this == TOP ? 2 : 1;
	}
	...
	private static org.hotswap.agent.javassist.CtClass findCommonSuperClass(org.hotswap.agent.javassist.CtClass one,
	...
	private Type mergeClasses(Type type) throws org.hotswap.agent.javassist.NotFoundException {
		...
		if (superClass.getSuperclass() == null) {
			Map<Object, CtClass> interfaces = findCommonInterfaces(type);
		...
		Map<Object, CtClass> commonDeclared = findExclusiveDeclaredInterfaces(type, superClass);
	...
	private Map<Object, CtClass> findCommonInterfaces(Type type) {
		Map<Object, CtClass> typeMap = getAllInterfaces(type.clazz, null);
		Map<Object, CtClass> thisMap = getAllInterfaces(this.clazz, null);

		return findCommonInterfaces(typeMap, thisMap);
	}

	private Map<Object, CtClass> findExclusiveDeclaredInterfaces(Type type,
			org.hotswap.agent.javassist.CtClass exclude) {
		Map<Object, CtClass> typeMap = getDeclaredInterfaces(type.clazz, null);
		Map<Object, CtClass> thisMap = getDeclaredInterfaces(this.clazz, null);
		Map<Object, CtClass> excludeMap = getAllInterfaces(exclude, null);

		Iterator<Object> i = excludeMap.keySet().iterator();
		while (i.hasNext()) {
			Object intf = i.next();
			typeMap.remove(intf);
			thisMap.remove(intf);
	...
	Map<Object, CtClass> findCommonInterfaces(Map<Object, CtClass> typeMap, Map<Object, CtClass> alterMap) {
		...
		i = new ArrayList<Object>(alterMap.values()).iterator();
		while (i.hasNext()) {
			...
			for (CtClass interface1 : interfaces) {
				alterMap.remove(interface1.getName());
			}
		}
	...
	Map<Object, CtClass> getAllInterfaces(org.hotswap.agent.javassist.CtClass clazz, Map<Object, CtClass> map) {
		if (map == null) {
			map = new HashMap<Object, CtClass>();
		}
	...
	Map<Object, CtClass> getDeclaredInterfaces(org.hotswap.agent.javassist.CtClass clazz, Map<Object, CtClass> map) {
		if (map == null) {
			map = new HashMap<Object, CtClass>();
		}
