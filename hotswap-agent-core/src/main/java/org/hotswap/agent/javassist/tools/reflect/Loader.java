 ...
 * 		cl.makeReflective("Person", "MyMetaobject", "ClassMetaobject");
 * 		cl.run("MyApp", args);
 * 	}
 * }
 * </pre>
 ...
 * loaded by <code>Loader</code> whereas the class <code>Main2</code> is not.
 * Thus, <code>Main</code> belongs to the same name space (security domain) as
 * <code>MyApp</code> whereas <code>Main2</code> does not; <code>Main2</code>
 * belongs to the same name space as <code>Loader</code>. For more details, see
 * the notes in the manual page of <code>Loader</code>.
 * <p/>
 * <p>
 ...
 * 		Loader cl = new Loader(ClassPool.getDefault(reflection));
 * 		reflection.makeReflective("Person", "MyMetaobject", "ClassMetaobject");
 * 		cl.run("MyApp", args);
 * 	}
 * }
 * </pre>
 ...
 * <p/>
 * <p>
 * <code>Loader</code> does not make a class reflective if that class is in a
 * <code>java.*</code> or <code>javax.*</code> pacakge because of the
 * specifications on the class loading algorithm of Java. The JVM does not allow
 * to load such a system class with a user class loader.
 * <p/>
 * <p>
 * To avoid this limitation, those classes should be statically modified with
 * <code>Compiler</code> and the original class files should be replaced.
 *
 * @see Reflection
 * @see Compiler
