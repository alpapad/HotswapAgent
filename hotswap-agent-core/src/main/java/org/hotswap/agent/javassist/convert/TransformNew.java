...
final public class TransformNew extends Transformer {
	...
	public int transform(CtClass clazz, int pos, org.hotswap.agent.javassist.bytecode.CodeIterator iterator,
		...
		if (c == NEW) {
			...
			if (cp.getClassInfo(index).equals(classname)) {
				...
				org.hotswap.agent.javassist.bytecode.StackMapTable smt = (org.hotswap.agent.javassist.bytecode.StackMapTable) iterator
						.get().getAttribute(org.hotswap.agent.javassist.bytecode.StackMapTable.tag);
				if (smt != null) {
					smt.removeNew(pos);
				}

				org.hotswap.agent.javassist.bytecode.StackMap sm = (org.hotswap.agent.javassist.bytecode.StackMap) iterator
						.get().getAttribute(org.hotswap.agent.javassist.bytecode.StackMap.tag);
				if (sm != null) {
					sm.removeNew(pos);
				}
			}
	...
	private int computeMethodref(int typedesc, org.hotswap.agent.javassist.bytecode.ConstPool cp) {
		...
		typedesc = cp.addUtf8Info(
				org.hotswap.agent.javassist.bytecode.Descriptor.changeReturnType(classname, cp.getUtf8Info(typedesc)));
