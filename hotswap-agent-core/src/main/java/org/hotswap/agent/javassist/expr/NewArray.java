...
public class NewArray extends Expr {
	...
	protected NewArray(int pos, org.hotswap.agent.javassist.bytecode.CodeIterator i,
			org.hotswap.agent.javassist.CtClass declaring, org.hotswap.agent.javassist.bytecode.MethodInfo m, int op) {
	...
	public org.hotswap.agent.javassist.CtClass getComponentType() throws org.hotswap.agent.javassist.NotFoundException {
		...
		} else if (opcode == ANEWARRAY || opcode == MULTIANEWARRAY) {
	...
	public int getDimension() {
		if (opcode == NEWARRAY) {
			...
			int index = iterator.u16bitAt(currentPos + 1);
			String desc = getConstPool().getClassInfo(index);
			return org.hotswap.agent.javassist.bytecode.Descriptor.arrayDimension(desc) + (opcode == ANEWARRAY ? 1 : 0);
		} else {
			throw new RuntimeException("bad opcode: " + opcode);
		}
	}
	...
	private void replace2(String statement)
			throws org.hotswap.agent.javassist.compiler.CompileError, org.hotswap.agent.javassist.NotFoundException,
			org.hotswap.agent.javassist.bytecode.BadBytecode, org.hotswap.agent.javassist.CannotCompileException {
		...
		if (opcode == NEWARRAY) {
			...
			org.hotswap.agent.javassist.CtPrimitiveType cpt = (org.hotswap.agent.javassist.CtPrimitiveType) getPrimitiveType(
					index);
	...
	static class ProceedForArray implements org.hotswap.agent.javassist.compiler.ProceedHandler {
		...
		@Override
		public void doit(org.hotswap.agent.javassist.compiler.JvstCodeGen gen,
				org.hotswap.agent.javassist.bytecode.Bytecode bytecode, ASTList args)
			...
			if (num != dimension) {
				throw new org.hotswap.agent.javassist.compiler.CompileError(
						org.hotswap.agent.javassist.compiler.Javac.proceedName
