 ...
 * <code>org.hotswap.agent.javassist.Loader</code> and classes included in
 * package <code>java.*</code> and <code>javax.*</code> to the parent class
 ...
 *
 * @see org.hotswap.agent.javassist.ClassPool
 * @see org.hotswap.agent.javassist.Translator
 */
public class Loader extends ClassLoader {
	private Hashtable<String, Loader> notDefinedHere; // must be atomic.
	...
	private void init(ClassPool cp) {
		notDefinedHere = new Hashtable<String, Loader>();
	 ...
	 *            target {@code main()}.
	 *
	 * @see org.hotswap.agent.javassist.Loader#run(String[])
	 */
	public static void main(String[] args) throws Throwable {
	...
	private boolean notDelegated(String name) {
		...
		for (int i = 0; i < n; ++i) {
			if (name.startsWith(notDefinedPackages.elementAt(i))) {
				return true;
			}
		}
