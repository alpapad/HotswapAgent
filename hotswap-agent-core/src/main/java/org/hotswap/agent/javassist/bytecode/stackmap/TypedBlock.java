...
public class TypedBlock extends BasicBlock {
	 ...
	 *            method returns null.
	 */
	public static TypedBlock[] makeBlocks(org.hotswap.agent.javassist.bytecode.MethodInfo minfo,
			org.hotswap.agent.javassist.bytecode.CodeAttribute ca, boolean optimize)
	...
	public void resetNumLocals() {
		if (localsTypes != null) {
			...
			while (nl > 0
					&& localsTypes[nl - 1].isBasicType() == org.hotswap.agent.javassist.bytecode.stackmap.TypeTag.TOP) {
				if (nl > 1) {
	...
	void initFirstBlock(int maxStack, int maxLocals, String className, String methodDesc, boolean isStatic,
			boolean isConstructor) throws org.hotswap.agent.javassist.bytecode.BadBytecode {
		...
		try {
			...
			throw new org.hotswap.agent.javassist.bytecode.BadBytecode("bad method descriptor: " + methodDesc);
		}
