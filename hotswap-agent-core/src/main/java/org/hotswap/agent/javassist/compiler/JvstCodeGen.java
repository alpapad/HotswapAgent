...
public class JvstCodeGen extends MemberCodeGen {
	...
	public JvstCodeGen(org.hotswap.agent.javassist.bytecode.Bytecode b, org.hotswap.agent.javassist.CtClass cc,
	...
	protected void atAssignParamList(org.hotswap.agent.javassist.CtClass[] params,
			org.hotswap.agent.javassist.bytecode.Bytecode code) throws CompileError {
		if (params == null) {
			return;
		}
	...
	protected void atCflow(org.hotswap.agent.javassist.compiler.ast.ASTList cname) throws CompileError {
		...
		bytecode.addInvokevirtual("Cflow", "value", "()I");
	...
	public int getMethodArgsLength(org.hotswap.agent.javassist.compiler.ast.ASTList args) {
		...
		while (args != null) {
			...
			if (a instanceof org.hotswap.agent.javassist.compiler.ast.Member
					&& ((org.hotswap.agent.javassist.compiler.ast.Member) a).get().equals(pname)) {
	...
	void compileInvokeSpecial(org.hotswap.agent.javassist.compiler.ast.ASTree target, String classname,
			String methodname, String descriptor, org.hotswap.agent.javassist.compiler.ast.ASTList args)
					throws CompileError {
		target.accept(this);
	 ...
	 */
	public int recordReturnType(org.hotswap.agent.javassist.CtClass type, String castName, String resultName,
	 ...
	 */
	public int recordParams(org.hotswap.agent.javassist.CtClass[] params, boolean isStatic, String prefix,
			String paramVarName, String paramsName, SymbolTable tbl) throws CompileError {
	 ...
	 */
	public int recordParams(org.hotswap.agent.javassist.CtClass[] params, boolean isStatic, String prefix,
			String paramVarName, String paramsName, boolean use0, int paramBase, String target, SymbolTable tbl)
					throws CompileError {
		int varNo;
		...
		if (use0) {
			...
			org.hotswap.agent.javassist.compiler.ast.Declarator decl = new org.hotswap.agent.javassist.compiler.ast.Declarator(
					TokenId.CLASS, MemberResolver.javaToJvmName(target), 0, varNo++,
					new org.hotswap.agent.javassist.compiler.ast.Symbol(varName));
	...
	private int recordVar(org.hotswap.agent.javassist.CtClass cc, String varName, int varNo, SymbolTable tbl)
			throws CompileError {
		...
		org.hotswap.agent.javassist.compiler.ast.Declarator decl = new org.hotswap.agent.javassist.compiler.ast.Declarator(
				exprType, className, arrayDim, varNo, new org.hotswap.agent.javassist.compiler.ast.Symbol(varName));
		tbl.append(varName, decl);
	...
	public void recordVariable(String typeDesc, String varName, int varNo, SymbolTable tbl) throws CompileError {
		...
		org.hotswap.agent.javassist.compiler.ast.Declarator decl = new org.hotswap.agent.javassist.compiler.ast.Declarator(
				type, cname, dim, varNo, new org.hotswap.agent.javassist.compiler.ast.Symbol(varName));
		tbl.append(varName, decl);
	}
		...
		if (params == null) {
			...
			for (int i = 0; i < n; ++i) {
				...
				if (params[i].isPrimitive()) {
					...
					regno += s;
					args[0] = pt;
					code.addInvokespecial(wrapper, "<init>", org.hotswap.agent.javassist.bytecode.Descriptor
							.ofMethod(org.hotswap.agent.javassist.CtClass.voidType, args));
					// invokespecial
				} else {
	...
	protected void compileUnwrapValue(org.hotswap.agent.javassist.CtClass type,
			org.hotswap.agent.javassist.bytecode.Bytecode code) throws CompileError {
