...
public class ClassFilePrinter {
	...
	public static void print(ClassFile cf, PrintWriter out) {
		...
		for (int i = 0; i < n; ++i) {
			...
			out.println(Modifier.toString(AccessFlag.toModifier(acc)) + " " + finfo.getName() + "\t"
					+ finfo.getDescriptor());
		...
		for (int i = 0; i < n; ++i) {
			...
			out.println(Modifier.toString(AccessFlag.toModifier(acc)) + " " + minfo.getName() + "\t"
					+ minfo.getDescriptor());
	...
	static void printAttributes(List<AttributeInfo> list, PrintWriter out, char kind) {
		...
		for (int i = 0; i < n; ++i) {
			...
			if (ai instanceof CodeAttribute) {
				...
				out.println("max stack " + ca.getMaxStack() + ", max locals " + ca.getMaxLocals() + ", "
				...
				org.hotswap.agent.javassist.bytecode.StackMapTable.Printer
						.print((org.hotswap.agent.javassist.bytecode.StackMapTable) ai, out);
