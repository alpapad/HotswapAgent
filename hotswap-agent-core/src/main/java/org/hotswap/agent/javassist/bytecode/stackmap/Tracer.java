...
public abstract class Tracer implements TypeTag {
	...
	private void doLDC(int index) {
		...
		if (tag == ConstPool.CONST_String) {
			stackTypes[stackTop++] = new org.hotswap.agent.javassist.bytecode.stackmap.TypeData.ClassName(
					"java.lang.String");
	...
	private int doOpcode148_201(int pos, byte[] code, int op) throws BadBytecode {
		...
		case Opcode.NEW: {
			...
			stackTypes[stackTop++] = new org.hotswap.agent.javassist.bytecode.stackmap.TypeData.UninitData(pos,
	...
	private int doInvokeMethod(int pos, byte[] code, boolean notStatic) throws BadBytecode {
		...
		if (notStatic) {
			...
			if (target instanceof org.hotswap.agent.javassist.bytecode.stackmap.TypeData.UninitTypeVar
					&& target.isUninit()) {
				constructorCalled(target,
						((org.hotswap.agent.javassist.bytecode.stackmap.TypeData.UninitTypeVar) target).offset());
			} else if (target instanceof org.hotswap.agent.javassist.bytecode.stackmap.TypeData.UninitData) {
				constructorCalled(target,
						((org.hotswap.agent.javassist.bytecode.stackmap.TypeData.UninitData) target).offset());
			}
	...
	private void pushMemberType(String descriptor) {
		...
		case '[':
			types[index] = new org.hotswap.agent.javassist.bytecode.stackmap.TypeData.ClassName(
					descriptor.substring(top));
			break;
		case 'L':
			types[index] = new org.hotswap.agent.javassist.bytecode.stackmap.TypeData.ClassName(
					getFieldClassName(descriptor, top));
			break;
		case 'J':
			types[index] = LONG;
			types[index + 1] = TOP;
