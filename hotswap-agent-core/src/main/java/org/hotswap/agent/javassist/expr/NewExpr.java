...
public class NewExpr extends Expr {
	 ...
	 */
	protected NewExpr(int pos, org.hotswap.agent.javassist.bytecode.CodeIterator i,
	 ...
	 */
	public org.hotswap.agent.javassist.CtConstructor getConstructor()
	...
	private int canReplace() throws org.hotswap.agent.javassist.CannotCompileException {
		...
		if (op == DUP) {
			return 4;
		} else if (op == DUP_X1 && iterator.byteAt(newPos + 4) == SWAP) {
			return 5;
		} else {
	...
	public void replace(String statement) throws org.hotswap.agent.javassist.CannotCompileException {
		...
		try {
			org.hotswap.agent.javassist.CtClass[] params = org.hotswap.agent.javassist.bytecode.Descriptor
					.getParameterTypes(signature, cp);
	...
	static class ProceedForNew implements org.hotswap.agent.javassist.compiler.ProceedHandler {
		...
		public void doit(org.hotswap.agent.javassist.compiler.JvstCodeGen gen,
			...
			gen.atMethodCallCore(newType, org.hotswap.agent.javassist.bytecode.MethodInfo.nameInit, args, false, true,
					-1, null);
			gen.setType(newType);
		}
