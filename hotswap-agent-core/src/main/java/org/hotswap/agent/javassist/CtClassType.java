...
import org.hotswap.agent.javassist.CtMember.Cache;
...
class CtClassType extends CtClass {
	...
	public ClassFile getClassFile2() {
		...
		try {
			...
			if (!cf.getName().equals(qualifiedName)) {
				throw new RuntimeException("cannot find " + qualifiedName + ": " + cf.getName() + " found in "
	 ...
	 *
	 * @see org.hotswap.agent.javassist.CtClass#incGetCounter()
	 *
	...
	static boolean hasAnnotationType(String annotationTypeName, ClassPool cp, AnnotationsAttribute a1,
		...
		try {
			...
			try {
				...
				try {
					...
					return org.hotswap.agent.javassist.bytecode.annotation.AnnotationImpl.make(clazz.getClassLoader(),
							clazz, cp, anno);
				} catch (Throwable e3) {
