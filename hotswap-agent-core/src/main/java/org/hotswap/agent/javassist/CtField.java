...
public class CtField extends CtMember {
	...
	public CtField(CtField src, CtClass declaring) throws CannotCompileException {
		...
		java.util.ListIterator<AttributeInfo> iterator = src.fieldInfo.getAttributes().listIterator();
	 ...
	 * type variables. See
	 * {@link org.hotswap.agent.javassist.CtClass#setGenericSignature(String)}
	 * for a code sample.
	 *
	 * @param sig
	 ...
	 *
	 * <p>
	 * This method does not automatically update method bodies that access this
	 * field. They have to be explicitly updated. For example, if some method
	 * contains an expression {@code t.value} and the type of the variable
	 * {@code t} is changed by {@link #setType(CtClass)} from {@code int} to
	 * {@code double}, then {@ t.value} has to be modified since the bytecode of
	 ...
	 * format. See {@link org.hotswap.agent.javassist.bytecode.AttributeInfo}.
	 *
	 * @param name
	 *            attribute name
	 */
	...
	public static abstract class Initializer {
		 ...
		 *            the class instantiated for the initial value.
		 *
		 * @see org.hotswap.agent.javassist.CtField.Initializer#byNewArray(CtClass,int)
		 * @see org.hotswap.agent.javassist.CtField.Initializer#byNewArray(CtClass,int[])
		 */
		...
		abstract int compile(CtClass type, String name, Bytecode code, CtClass[] parameters, Javac drv)
