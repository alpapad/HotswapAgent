...
public class AccessorMaker {
	...
	public String getConstructor(org.hotswap.agent.javassist.CtClass c, String desc,
			org.hotswap.agent.javassist.bytecode.MethodInfo orig) throws CompileError {
		...
		org.hotswap.agent.javassist.bytecode.ClassFile cf = clazz.getClassFile(); // turn
																					// on
																					// the
																					// modified
																					// flag.
		try {
			...
			org.hotswap.agent.javassist.bytecode.MethodInfo minfo = new org.hotswap.agent.javassist.bytecode.MethodInfo(
					cp, org.hotswap.agent.javassist.bytecode.MethodInfo.nameInit, consDesc);
			...
			org.hotswap.agent.javassist.CtClass[] params = org.hotswap.agent.javassist.bytecode.Descriptor
					.getParameterTypes(desc, pool);
	...
	public String getMethodAccessor(String name, String desc, String accDesc,
		...
		try {
			...
			org.hotswap.agent.javassist.CtClass[] params = org.hotswap.agent.javassist.bytecode.Descriptor
					.getParameterTypes(accDesc, pool);
			int regno = 0;
			org.hotswap.agent.javassist.bytecode.Bytecode code = new org.hotswap.agent.javassist.bytecode.Bytecode(cp);
			for (CtClass param : params) {
				regno += code.addLoad(regno, param);
	 ...
	 */
	public org.hotswap.agent.javassist.bytecode.MethodInfo getFieldGetter(
			org.hotswap.agent.javassist.bytecode.FieldInfo finfo, boolean is_static) throws CompileError {
		String fieldName = finfo.getName();
		String key = fieldName + ":getter";
		Object res = accessors.get(key);
		if (res != null) {
			return (org.hotswap.agent.javassist.bytecode.MethodInfo) res; // already
																			// exists.
		}
	 ...
	 */
	public org.hotswap.agent.javassist.bytecode.MethodInfo getFieldSetter(
			org.hotswap.agent.javassist.bytecode.FieldInfo finfo, boolean is_static) throws CompileError {
		String fieldName = finfo.getName();
		...
		try {
			...
			if (is_static) {
				...
				code.addAload(0);
				reg = code.addLoad(1, org.hotswap.agent.javassist.bytecode.Descriptor.toCtClass(fieldType, pool)) + 1;
