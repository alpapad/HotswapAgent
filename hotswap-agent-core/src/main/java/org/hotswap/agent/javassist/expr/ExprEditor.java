...
public class ExprEditor {
	 ...
	 */
	public boolean doit(org.hotswap.agent.javassist.CtClass clazz,
			org.hotswap.agent.javassist.bytecode.MethodInfo minfo) throws CannotCompileException {
		...
		int n = et.size();
		for (int i = 0; i < n; ++i) {
			org.hotswap.agent.javassist.expr.Handler h = new org.hotswap.agent.javassist.expr.Handler(et, i, iterator,
					clazz, minfo);
			edit(h);
			if (h.edited()) {
				edited = true;
	...
	final boolean loopBody(org.hotswap.agent.javassist.bytecode.CodeIterator iterator,
			org.hotswap.agent.javassist.CtClass clazz, org.hotswap.agent.javassist.bytecode.MethodInfo minfo,
			LoopContext context) throws CannotCompileException {
		try {
			Expr expr = null;
			int pos = iterator.next();
			int c = iterator.byteAt(pos);

			if (c < org.hotswap.agent.javassist.bytecode.Opcode.GETSTATIC) {
				/* skip */ ;
			} else if (c < org.hotswap.agent.javassist.bytecode.Opcode.NEWARRAY) { // c
																					// <
																					// 188
				if (c == org.hotswap.agent.javassist.bytecode.Opcode.INVOKESTATIC
				...
				} else if (c == org.hotswap.agent.javassist.bytecode.Opcode.GETFIELD
						|| c == org.hotswap.agent.javassist.bytecode.Opcode.GETSTATIC
					...
					if (newList != null
							&& minfo.getConstPool().isConstructor(newList.type, iterator.u16bitAt(pos + 1)) > 0) {
				...
				if (c == org.hotswap.agent.javassist.bytecode.Opcode.NEWARRAY
						|| c == org.hotswap.agent.javassist.bytecode.Opcode.ANEWARRAY
