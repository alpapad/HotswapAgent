...
public class AppletServer extends org.hotswap.agent.javassist.tools.web.Webserver {
	 ...
	 *            port number
	 */
	public AppletServer(String port) throws IOException, org.hotswap.agent.javassist.NotFoundException,
	 ...
	 *            port number
	 */
	public AppletServer(int port) throws IOException, org.hotswap.agent.javassist.NotFoundException,
			org.hotswap.agent.javassist.CannotCompileException {
	 ...
	 */
	public AppletServer(int port, org.hotswap.agent.javassist.ClassPool src) throws IOException,
			org.hotswap.agent.javassist.NotFoundException, org.hotswap.agent.javassist.CannotCompileException {
		this(new org.hotswap.agent.javassist.ClassPool(src), new StubGenerator(), port);
	}

	private AppletServer(org.hotswap.agent.javassist.ClassPool loader, StubGenerator gen, int port) throws IOException,
			org.hotswap.agent.javassist.NotFoundException, org.hotswap.agent.javassist.CannotCompileException {
		super(port);
	...
	private void processRMI(InputStream ins, OutputStream outs) throws IOException {
		...
		Object rvalue = null;
		try {
			ExportedObject eo = exportedObjects.elementAt(objectId);
	...
	private Object[] readParameters(ObjectInputStream in) throws IOException, ClassNotFoundException {
		...
		for (int i = 0; i < n; ++i) {
			...
			if (a instanceof RemoteRef) {
				...
				ExportedObject eo = exportedObjects.elementAt(ref.oid);
				a = eo.object;
			}
	...
	private Object convertRvalue(Object rvalue) throws org.hotswap.agent.javassist.CannotCompileException {
		if (rvalue == null) {
