...
public class Reflection implements org.hotswap.agent.javassist.Translator {
	...
	static final String classMetaobjectClassName = "ClassMetaobject";
	...
	private boolean isExcluded(String name) {
		return name.startsWith(org.hotswap.agent.javassist.tools.reflect.ClassMetaobject.methodPrefix)
				|| name.equals(classobjectAccessor) || name.equals(metaobjectSetter) || name.equals(metaobjectGetter)
				|| name.startsWith(readPrefix) || name.startsWith(writePrefix);
	}
	...
	public void start(org.hotswap.agent.javassist.ClassPool pool) throws org.hotswap.agent.javassist.NotFoundException {
		classPool = pool;
		final String msg = "Sample is not found or broken.";
		try {
			...
			readParam = new org.hotswap.agent.javassist.CtClass[] { classPool.get("java.lang.Object") };
	...
	private void registerReflectiveClass(org.hotswap.agent.javassist.CtClass clazz) {
		...
		for (CtField f : fs) {
			...
			if ((mod & org.hotswap.agent.javassist.Modifier.PUBLIC) != 0
					&& (mod & org.hotswap.agent.javassist.Modifier.FINAL) == 0) {
	...
	private boolean modifyClassfile(org.hotswap.agent.javassist.CtClass clazz,
			org.hotswap.agent.javassist.CtClass metaobject, org.hotswap.agent.javassist.CtClass metaclass)
		...
		if (addMeta) {
			f = new org.hotswap.agent.javassist.CtField(classPool.get("Metaobject"), metaobjectField, clazz);
		...
		f = new org.hotswap.agent.javassist.CtField(classPool.get("ClassMetaobject"), classobjectField, clazz);
		f.setModifiers(org.hotswap.agent.javassist.Modifier.PRIVATE | org.hotswap.agent.javassist.Modifier.STATIC);
		clazz.addField(f,
				org.hotswap.agent.javassist.CtField.Initializer.byNew(metaclass, new String[] { clazz.getName() }));
	...
	private void processMethods(org.hotswap.agent.javassist.CtClass clazz, boolean dontSearch)
		...
		for (int i = 0; i < ms.length; ++i) {
			...
			if (org.hotswap.agent.javassist.Modifier.isPublic(mod)
					&& !org.hotswap.agent.javassist.Modifier.isAbstract(mod)) {
	...
	private void processMethods0(int mod, org.hotswap.agent.javassist.CtClass clazz,
		...
		m2.setName(org.hotswap.agent.javassist.tools.reflect.ClassMetaobject.methodPrefix + identifier + "_" + name);
		...
		org.hotswap.agent.javassist.CtMethod wmethod = org.hotswap.agent.javassist.CtNewMethod.wrapped(
				m.getReturnType(), name, m.getParameterTypes(), m.getExceptionTypes(), body,
				ConstParameter.integer(identifier), clazz);
		wmethod.setModifiers(mod);
		clazz.addMethod(wmethod);
	}

	private org.hotswap.agent.javassist.CtMethod findOriginal(org.hotswap.agent.javassist.CtMethod m,
			boolean dontSearch) throws org.hotswap.agent.javassist.NotFoundException {
		if (dontSearch) {
			return m;
		}
	...
	private void processFields(org.hotswap.agent.javassist.CtClass clazz)
		...
		for (CtField f : fs) {
			...
			if ((mod & org.hotswap.agent.javassist.Modifier.PUBLIC) != 0
				...
				org.hotswap.agent.javassist.CtMethod wmethod = org.hotswap.agent.javassist.CtNewMethod.wrapped(ftype,
						readPrefix + name, readParam, null, trapRead, ConstParameter.string(name), clazz);
				wmethod.setModifiers(mod);
				clazz.addMethod(wmethod);
				...
				wmethod = org.hotswap.agent.javassist.CtNewMethod.wrapped(org.hotswap.agent.javassist.CtClass.voidType,
						writePrefix + name, writeParam, null, trapWrite, ConstParameter.string(name), clazz);
				wmethod.setModifiers(mod);
				clazz.addMethod(wmethod);
			}
		}
