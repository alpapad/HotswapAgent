...
/**
 * Invoke methods on plugin manager, avoid classloader conflicts. Each method
 * has two variants - direct call or method source code builder.
 * <p/>
 * Note that because methods are invoked accross classloader, only parameters
 * known to both classloaders can be used. This is generally true for basic and
 * java.* types.
 *
 * @author Jiri Bubnik
 */
public class PluginManagerInvoker {
	 ...
	 *            classloader in which the plugin should reside
	 */
	@SuppressWarnings("unchecked")
	public static <T> T callInitializePlugin(Class<T> pluginClass, ClassLoader appClassLoader) {
		return (T) PluginManager.getInstance().getPluginRegistry().initializePlugin(pluginClass.getName(), appClassLoader);
	}
	...
	/**
	 * Methods on plugin should be called via reflection, because the real
	 * plugin object is in parent classloader, but plugin class may be defined
	 * in app classloader as well introducing ClassCastException on same class
	 * name.
	 *
	 * @param pluginClass
	 *            class name of the plugin - it is used to resolve plugin
	 *            instance from plugin manager
	 * @param appClassLoader
	 *            application classloader (to resolve plugin instance)
	 * @param method
	 *            method name
	 * @param paramTypes
	 *            param types (as required by reflection)
	 * @param params
	 *            actual param values
	 ...
	 * @param method
	 *            method name
	 * @param paramValueAndType
	 *            for each param its value AND type must be provided
	...
	/**
	 * Same as
	 * {@link PluginManagerInvoker#buildCallPluginMethod(Class, String, String...)}
	 * , but with explicit appClassLoader variable. Use this method if
	 * appClassLoader is different from getClass().getClassLoader().
	 */
	public static String buildCallPluginMethod(String appClassLoaderVar, Class<?> pluginClass, String method,
		...
		// Object __pluginInstance =
		// org.hotswap.agent.config.PluginManager.getInstance().getPlugin(org.hotswap.agent.plugin.TestPlugin.class.getName(),
		// __pluginClassLoader);
		...
		// Class __pluginClass =
		// __pluginClassLoader.loadClass("org.hotswap.agent.plugin.TestPlugin");
		...
		for (int i = 0; i < paramCount; i++) {
			// paramTypes[i] = =
			// __pluginClassLoader.loadClass("my.test.TestClass").getClass();
			b.append(
					"paramTypes[" + i + "] = __pluginClassLoader.loadClass(\"" + paramValueAndType[i * 2 + 1] + "\");");
		}

		// java.lang.reflect.Method __pluginMethod =
		// __pluginClass.getDeclaredMethod("method", paramType1, paramType2);
