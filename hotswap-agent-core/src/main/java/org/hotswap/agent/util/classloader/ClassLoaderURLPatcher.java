...
/**
 * Add agent JAR URL to a classloader. This works only for webapp classloader of
 * URLClassLoader type and webapp first precedence.
 *
 * @author Jiri Bubnik
 */
@Deprecated
public class ClassLoaderURLPatcher implements ClassLoaderPatcher {
	...
	@Override
	public void patch(ClassLoader classLoaderFrom, String pluginPath, ClassLoader classLoader,
			...
			synchronized (patchedClassLoaders) {
				if (!patchedClassLoaders.contains(classLoader)) {
					...
					if (classLoader instanceof URLClassLoader) {
						...
						LOGGER.debug(
								"Unable to patch ClassLoader {} of type {}. Only URLClassLoader(s) can be currently patched.",
	...
	/**
	 * Resolve JAR file path in format:
	 * "jar:file:/J:/HotSwapAgent/target/HotSwapAgent-1.0-SNAPSHOT.jar!/" to a
	 * valid URL file:/J:/HotSwapAgent/target/HotSwapAgent-1.0-SNAPSHOT.jar
	 *
	 * @param url
	 *            URL to resolve
	 * @return modified URL
