...
public class ReflectionHelper {
	...
	/**
	 * Convenience wrapper to reflection method invoke API. Invoke the method
	 * and hide checked exceptions.
	 *
	 * @param target
	 *            object to invoke the method on (or null for static methods)
	 * @param clazz
	 *            class name
	 * @param methodName
	 *            method name
	 * @param parameterTypes
	 *            parameter types to resolve method name
	 * @param args
	 *            actual arguments
	 ...
	 *             for InvocationTargetException (exception in invoked method)
	 */
	public static Object invoke(Object target, Class<?> clazz, String methodName, Class<?>[] parameterTypes,
		...
		try {
			...
			throw new IllegalArgumentException(String.format("Illegal arguments method %s.%s(%s) on %s, params %s",
					clazz.getName(), methodName, Arrays.toString(parameterTypes), target, Arrays.toString(args)), e);
	...
	/**
	 * Convenience wrapper to reflection method invoke API. Get field value and
	 * hide checked exceptions. Field class is set by
	 *
	 * @param target
	 *            object to get field value (or null for static methods)
	 * @param fieldName
	 *            field name
	 * @return field value
	 * @throws IllegalArgumentException
	...
	public static <T> T get(Object target, String fieldName) {
		...
		if (clazz == null) {
			throw new IllegalArgumentException(
					String.format("No such field %s.%s on %s", target.getClass(), fieldName, target));
		}

		return (T)get(target, clazz, fieldName);
	}
	...
	public static Object get(Object target, Class<?> clazz, String fieldName) {
		try {
			...
			throw new IllegalArgumentException(
					String.format("No such field %s.%s on %s", clazz.getName(), fieldName, target), e);
		} catch (IllegalAccessException e) {
			throw new IllegalArgumentException(
					String.format("Illegal access field %s.%s on %s", clazz.getName(), fieldName, target), e);
		}
	}

	/**
	 * Convenience wrapper to reflection method invoke API. Get field value and
	 * swallow exceptions. Use this method if you have multiple framework
	 * support and the field may not exist in current version.
	 *
	 * @param target
	 *            object to get field value (or null for static methods)
	 * @param clazz
	...
	/**
	 * Convenience wrapper to reflection method invoke API. Set field value and
	 * hide checked exceptions.
	 *
	 * @param target
	 *            object to get field value (or null for static methods)
