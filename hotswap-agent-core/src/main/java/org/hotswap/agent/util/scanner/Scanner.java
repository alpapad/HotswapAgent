...
public interface Scanner {
	/**
	 * Scan all directories matching path (there might be multiple locations on
	 * classpath, JAR directory, ...) and call visitor for each found file.
	 *
	 * @param classLoader
	 *            classloader to resolve path
	 * @param path
	 *            a '/'-separated path name that identifies the resource
	 *            directory. Semantics same as
	 *            {@link java.lang.ClassLoader#getResources}.
	 * @param visitor
	 *            visit each file on the path
	 * @throws IOException
	 *             any IO exception while scanning
	 */
