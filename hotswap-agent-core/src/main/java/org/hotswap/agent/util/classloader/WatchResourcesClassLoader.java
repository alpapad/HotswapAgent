...
public class WatchResourcesClassLoader extends URLClassLoader {
	...
	/**
	 * Watch for requested resource in parent classloader in case it is not
	 * found by this classloader? Note that there is child first precedence
	 * anyway.
	 */
	...
	/**
	 * URL classloader configured to get resources only from exact set of URL's
	 * (no parent delegation)
	 */
					...
					public void onEvent(WatchFileEvent event) {
						try {
							if (event.isFile() || event.isDirectory()) {
								...
								LOGGER.trace(
										"File '{}' changed and will be returned instead of original classloader equivalent.",
										event.getURI().toURL());
							}
	...
	/**
	 * Returns URL only if the resource is found in changedURL and was actually
	 * changed after instantiation of this classloader.
	 */
	@Override
	 ...
	 * conflicting requirements for other resources inclusion. This class should
	 * "hide" the original resource, hence it should not be included in the
	 * resoult. On the other hand, there may be resource with the same name in
	 * other JAR which should be included and now is hidden (for example
	 * multiple persistence.xml). Maybe a new property to influence this
	 * behaviour?
	 */
	@Override
	public Enumeration<URL> getResources(String name) throws IOException {
		...
		// if extraClasspath contains at least one element, return only
		// extraClasspath
