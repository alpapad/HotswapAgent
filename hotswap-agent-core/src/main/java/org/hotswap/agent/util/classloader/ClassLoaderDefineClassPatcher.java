...
/**
 * Classloader patch which will redefine each patch via Javassist in the target
 * classloader.
 * <p/>
 * Note that the class will typically be already accessible by parent
 * classloader, but if it is loaded from parent classloader, it does not have
 * access to other child classloader classes.
 * <p/>
 * Redefine will work only if the class was not loaded by the child classloader.
 * This may not be used for Plugin class itself, because some target library
 * classes may be enhanced by plugin reference (e.g. to set some initialized
 * property). Although the class resides in parent classloader it cannot be
 * redefined in child classloader with other definition - the classloader
 * already knows about this class. This is the reason, why plugin class cannot
 * be executed in child classloader.
 *
 * @author Jiri Bubnik
 */
		...
		if (cache != null) {
			...
			for (byte[] pluginBytes : cache) {
				try {
					...
					try {
						// force to load class in classLoaderFrom (it may not
						// yet be loaded) and if the classLoaderTo
						// is parent of classLoaderFrom, after definition in
						// classLoaderTo will classLoaderFrom return
						// class from parent classloader instead own definition
						// (hence change of behaviour).
						...
						// and load the class in classLoaderTo as well. NOw the
						// class is defined in BOTH classloaders.
					...
					LOGGER.trace("Skipping class definition app classloader {} - " + "unknown error.", e,
							classLoaderTo);
				}
			}
		}

		LOGGER.debug("Classloader {} patched with plugin classes from agent classloader {}.", classLoaderTo,
				classLoaderFrom);
				...
				try {
					scanner.scan(classLoaderFrom, pluginPath, new ScannerVisitor() {
						...
						public void visit(InputStream file) throws IOException {
							...
							// skip plugin classes
							// TODO this should be skipped only in patching
							// application classloader. To copy
							// classes into agent classloader, Plugin class must
							// be copied as well
							// if (patchClass.hasAnnotation(Plugin.class)) {
							// LOGGER.trace("Skipping plugin class: " +
							// patchClass.getName());
							// return;
							// }
