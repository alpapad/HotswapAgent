...
public class PluginRegistry {
	...
	/**
	 * Create an instanec of plugin registry and initialize scanner and
	 * processor.
	 */
	...
	/**
	 * Scan for plugins by @Plugin annotation on PLUGIN_PATH and process plugin
	 * annotations.
	 *
	 * @param classLoader
	 *            classloader to resolve plugin package. This will be used by
	 *            annotation scanner.
	 * @param pluginPackage
	 *            the package to be searched (e.g. org.agent.hotswap.plugin)
	 */
	public void scanPlugins(ClassLoader classLoader, String pluginPackage) {
		...
		try {
			...
			// Plugin class must be always defined directly in the agent
			// classloader, otherwise it will not be available
			...
			for (String discoveredPlugin : discoveredPlugins) {
				LOGGER.debug("Scanning plugin: '" + discoveredPlugin );
				...
				LOGGER.debug("Found plugin: '" + pluginName );

				// check for duplicate plugin definition. It may happen if class
				...
				if (annotationProcessor.processStaticAnnotations(pluginClass, pluginClass)) {
	 ...
	 *
	 * @param pluginClass
	 *            class of plugin to instantiate
	 * @param appClassLoader
	 *            target application classloader
	...
	public Object initializePlugin(String pluginClass, ClassLoader appClassLoader) {
		...
		if (annotationProcessor.processPluginInstanceAnnotations(pluginInstance, appClassLoader, pluginManager.getPluginConfiguration(appClassLoader))) {
	 ...
	 *            type of the plugin
	 * @param classLoader
	 *            classloader of the plugin
	 * @param <T>
	 *            type of the plugin to return correct instance.
	 * @return the plugin
	 * @throws IllegalArgumentException
	 *             if classLoader not initialized or plugin not found
	 */
	...
	public <T> T getPlugin(Class<T> pluginClass, ClassLoader classLoader) {
		...
		for (Map.Entry<ClassLoader, ?> registeredClassLoaderEntry : registeredPlugins.get(pluginClass).entrySet()) {
		...
		throw new IllegalArgumentException(
				String.format("Plugin %s is not initialized in classloader %s.", pluginClass, classLoader));
	}
	 ...
	 *            classloader of the plugin
	 * @param checkParent
	 *            for parent classloaders as well?
	 * @return true/false
	 */
	public boolean hasPlugin(Class<?> pluginClass, ClassLoader classLoader, boolean checkParent) {
		...
		for (Map.Entry<ClassLoader, Object> registeredClassLoaderEntry : registeredPlugins.get(pluginClass)
				.entrySet()) {
	...
	public ClassLoader getAppClassLoader(Object plugin) {
		// search with for loop. Maybe performance improvement to create reverse
		// map if this is used heavily
