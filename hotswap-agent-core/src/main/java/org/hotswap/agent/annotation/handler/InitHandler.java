 ...
 * <p/>
 * The {org.hotswap.agent.annotation.Init} annotation can be set on field or
 * method and static or non static. See the annotation description for usage
 * info.
 *
 * @author Jiri Bubnik
...
public class InitHandler implements PluginHandler<Init> {
	...
	public boolean initField(PluginAnnotation<Init> pluginAnnotation) {
		...
		// if plugin not set, it is static method on plugin registration, use
		// agent classloader
		...
		try {
			...
			LOGGER.error("Unable to set plugin field '{}' to value '{}' on plugin '{}'", e, field.getName(), value,
	...
	// If static, just register callback, otherwise the method is immediately
	// invoked.
	@Override
	...
	// resolve all method parameter types to actual values and invoke the plugin
	// method (both static and non static)
	private boolean invokeInitMethod(PluginAnnotation<Init> pluginAnnotation, Object plugin, ClassLoader classLoader) {
		...
		try {
			...
			LOGGER.error("InvocationTargetException in init method on plugin {}.", e,
					pluginAnnotation.getPluginClass());
			return false;
		}
	}
	 ...
	 *
	 * @param pluginAnnotation
	 *            description of plugin method to call
	 * @return true if ok
	 */
	...
	protected Object resolveType(ClassLoader classLoader, Class<?> pluginClass, Class<?> type) {
		...
		if (type.isAssignableFrom(PluginManager.class)) {
			...
			LOGGER.error("Unable process @Init on plugin '{}'." + " Type '" + type
					+ "' is not recognized for @Init annotation.", pluginClass);
			return null;
		}
	}
}
