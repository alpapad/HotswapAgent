...
/**
 * Example HotSwapperJpda class from javaassist is copied to the plugin, becuse
 * it needs to reside in the application classloader to avoid NoClassDefFound
 * error on tools.jar classes. Otherwise it is the same code as in javassist.
 * <p/>
...
public class HotSwapperJpda {
	 ...
	 */
	public HotSwapperJpda(int port) throws IOException, IllegalConnectorArgumentsException {
	 ...
	 *            the port number used for the connection to the JVM.
	 */
	public HotSwapperJpda(String port) throws IOException, IllegalConnectorArgumentsException {
		jvm = null;
		request = null;
		newClassFiles = null;
		trigger = new Trigger();
	 ...
	 * <p/>
	 * This is mainly useful for unit testing - declare multiple version of a
	 * class and then hotswap definition and do the tests.
	 *
	 * @param original
	 *            original class currently in use
	 * @param swap
	 *            fully qualified class name of class to swap
	 * @throws Exception
	 *             swap exception
	 */
	public void swapClasses(Class<?> original, String swap) throws Exception {
		// need to recreate classpool on each swap to avoid stale class
		// definition
