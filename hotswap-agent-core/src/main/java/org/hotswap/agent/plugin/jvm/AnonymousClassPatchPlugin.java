...
/**
 * Class names MyClass$1, MyClass$2 are created in the order as anonymous class
 * appears in the source code. After anonymous class insertion/deletion the
 * indexes are shifted producing not compatible hot swap.
 * <p/>
 * This patch will create class state info before the change (from current
 * ClassLoader via reflection) and after the change (from filesystem using
 * javassist) find all compatible transitions.
 * <p/>
 * <p/>
 * For example if you exchange order the anonymous class appears in the source
 * code, Transition may produce something like:
 * <ul>
 ...
 * <ul>
 * <li>When the class MyClass$1 is hot swapped, the bytecode from MyClass$2 is
 * returned (and renamed to MyClass$1)</li>
 * <li>When the class MyClass$2 is hot swapped, the bytecode from MyClass$3 is
 * returned (and renamed to MyClass$2)</li>
 * <li>When the class MyClass$3 is hot swapped, the bytecode from MyClass$1 is
 * returned (and renamed to MyClass$3)</li>
 * <li>When the class MyClass is hot swapped, all occurences of MyClass$1 are
 * exchanged for MyClass$3</li>
 ...
 * </ul>
 * <p/>
 * Swap may produce even to not compatible change. Consider existing MyClass$1
 * and MyClass$2, then MyClass$1 is removed. Then hotswap is called only on
 * MyClass$1, which contains different class to MyClass$2. Then MyClass$1 is on
 * hotswap replaced with empty implementation and new class MyClass$1000x is
 * created to contain code from the new MyClass$1 (class compatible with old
 * MyClass$2). Not that because this is not a true hotswap, old existing
 * instances of MyClass$1 are updated to an empty class, not the new one. When
 * calling a method on this class, AbstractErrorMethod is thrown (this should be
 * replaced to some more clear error in the future).
 *
 * @author Jiri Bubnik
 */
@Plugin(name = "AnonymousClassPatch", description = "Swap anonymous inner class names to avoid not compatible changes.", testedVersions = {
		"DCEVM" })
public class AnonymousClassPatchPlugin {
	...
	// Map ClassLoader -> (className -> infos about inner/local anonymous
	// classes)
	// This caches information for one hotswap on main class and all anonymous
	// classes
	private static Map<ClassLoader, Map<String, AnonymousClassInfos>> anonymousClassInfosMap = new WeakHashMap<ClassLoader, Map<String, AnonymousClassInfos>>();

	/**
	 * Replace an anonymous class with an compatible change (from another class
	 * according to state info). If no compatible class exists, replace with
	 * compatible empty implementation.
	 */
	@OnClassLoadEvent(classNameRegexp = ".*\\$\\d+", events = LoadEvent.REDEFINE)
	public static CtClass patchAnonymousClass(ClassLoader classLoader, ClassPool classPool, String className,
			Class<?> original) throws IOException, NotFoundException, CannotCompileException {
		...
		if (compatibleName != null) {
			...
			LOGGER.debug("Anonymous class '{}' - not compatible change is replaced with empty implementation.",
					javaClass, compatibleName);

			// replace current class with empty implementation (to avid not
			...
			// throw new IllegalAccessError("HOTSWAP AGENT - obsolete anonymous
			// class. This class has been
			// replaced with a new version. Automatic update of old instances
			// containing references to obsolete
	...
	// new anonymous class, not covered by hotswap (patchAnonymousClass) -
	// register custom transformer and
	...
	/**
	 * If class contains anonymous classes, rename class references to
	 * compatible transition classes.
	 * <p/>
	 * If the transitioned class is not loaded by hotswap replace, catch class
	 * define event to do the replacement.
	 * <p/>
	 * Define new synthetic classes for not compatible changes.
	 */
	@OnClassLoadEvent(classNameRegexp = ".*", events = LoadEvent.REDEFINE)
	public static byte[] patchMainClass(String className, ClassPool classPool, ClassLoader classLoader,
			ProtectionDomain protectionDomain) throws IOException, CannotCompileException, NotFoundException {
		...
		for (Map.Entry<AnonymousClassInfo, AnonymousClassInfo> entry : transitions.entrySet()) {
			...
			if (isHotswapAgentSyntheticClass(compatibleName)) {
				LOGGER.debug("Anonymous class '{}' not comatible and is replaced with synthetic class '{}'", newName,
						compatibleName);
				// define contens of new class as new unique
				// "myClass$hotswapAgentXx" class
	...
	/**
	 * Calculate anonymous class new/previous state info from current
	 * classloader/filesystem. It checks, if state info is current via
	 * modification date on the main class file.
	 * <p/>
	 * <p/>
	 * Note: Synchronized may be too restrictive, in case of performance issues
	 * consider synchronization only on a classloader and class.
	 */
	private static synchronized AnonymousClassInfos getStateInfo(ClassLoader classLoader, ClassPool classPool,
			String className) {
	...
	/**
	 * Return classInfos for a classloader. Hold known classloaders in weak hash
	 * map.
	 */
