 ...
 * <p/>
 * This plugin creates an instance for each classloader with autoHotswap agent
 * property set. Then it listens for .class file change and executes hotswap via
 * JPDA API.
 *
 * @author Jiri Bubnik
 * @see HotSwapperJpda
 */
@Plugin(name = "Hotswapper", description = "Watch for any class file change and reload (hotswap) it on the fly.", testedVersions = {
...
public class HotswapperPlugin {
	...
	// command to do actual hotswap. Single command to merge possible multiple
	// reload actions.
...
//    }
	 ...
	 *            it can be run in any classloader with tools.jar on classpath.
	 *            AppClassLoader can be setup by maven dependency (jetty
	 *            plugin), use this classloader.
	 * @param port
	 *            attach the hotswapper
	 */
	public void initHotswapCommand(ClassLoader appClassLoader, String port) {
		LOGGER.debug("initHotswapCommand");
	...
	/**
	 * For each classloader check for autoHotswap configuration instance with
	 * hotswapper.
	 */
	@Init
	public static void init(Scheduler scheduler, PluginConfiguration pluginConfiguration, ClassLoader appClassLoader) {
