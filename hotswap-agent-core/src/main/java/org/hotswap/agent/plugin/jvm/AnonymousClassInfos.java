 ...
 * <p/>
 * This class will on construction search for all anonymous classes of the main
 * class and calculate superclass, interfaces, all methods signature and all
 * fields signature. Depending on used constructor this is done via reflection
 * from ClassLoader (current loaded state) or from ClassPool via javaassist.
 * Note that ClassPool uses LoadClassPath on the ClassLoader and hence are
 * resources resolved via the ClassLoader. Javaasist resolves the resource and
 * returns bytcode as is in the resource file.
 * <p/>
 * Use mapPreviousState() to create compatible transition mapping between old
 * state and new state. This mapping is then used by plugin to swap class
 * bytecoded to retain hotswap changes compatible
 *
 * @author Jiri Bubnik
 */
public class AnonymousClassInfos {
	...
	// start indexing hotswap created synthetic anonymous classes from this
	// index to avoid collision with existing
	public static final int UNIQUE_CLASS_START_INDEX = 10000;

	// how many milliseconds is delta is considered as same time in modification
	// check
	...
	public AnonymousClassInfos(ClassLoader classLoader, String className) {
		...
		try {
			// reflective call to check already loaded class (not to load a new
			// one)
	 ...
	 *
	 * @param classPool
	 *            classPool to resolve class files
	 * @param className
	 *            main class
	 */
	public AnonymousClassInfos(ClassPool classPool, String className) {
		...
		// search for declared classes in new state to skip obsolete anonymous
		// inner classes on filesystem
	 ...
	 *
	 * @return map previous -> new. If no mapping to previous exists, synthetic
	...
	private void calculateCompatibleTransitions() {
		...
		// try to match - exact, than signatures (enclosing method may change),
		// than class signature
		...
		for (AnonymousClassInfo currentNotMatched : currentInfos) {
			if (lastAnonymousClassIndex < newDefinitionCount) {
				// free anonymous class available - this will be registered with
				// onDefine event, +1 because one based index name
				compatibleTransitions.put(new AnonymousClassInfo(className + "$" + (lastAnonymousClassIndex + 1)),
						currentNotMatched);
	...
	/**
	 * Iterate through both lists and find matching anonymous classes using
	 * matcher. Found matches are removed from previous and current lists and
	 * added to transitions.
	 */
	private void searchForMappings(Map<AnonymousClassInfo, AnonymousClassInfo> transitions,
			List<AnonymousClassInfo> previousInfos, List<AnonymousClassInfo> currentInfos,
	 ...
	 *
	 * @param className
	 *            class name of the anonymous class (Should be in the form of
	 *            MyClass$3)
	 ...
	 * Usually new state is created from classpool, while old state from
	 * classloader.
	 *
	 * @param previousAnonymousClassInfos
	 *            previous state
	 */
	...
	/**
	 * Return true, if last modification timestamp is same as current timestamp
	 * of className.
	 *
	 * @param classPool
	 ...
	 * @return name of compatible new class that should replace old class. Can
	 *         be null if no compatible class found.
	 */
